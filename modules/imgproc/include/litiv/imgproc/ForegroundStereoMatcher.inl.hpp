...
inline void StereoSegmMatcher::GraphModelData::updateResegmModel(size_t nCamIdx, bool bInit) {
    ...
    for(size_t nGraphNodeIdx=0; nGraphNodeIdx<m_anValidGraphNodes[nCamIdx]; ++nGraphNodeIdx) {
        ...
        const ValueType tFGColorUnaryCost = ValueType(-std::log2(dColorFGProb)*STEREOSEGMATCH_IMGSIM_COST_COLOR_SCALE);
        ...
        const ValueType tBGColorUnaryCost = ValueType(-std::log2(dColorBGProb)*STEREOSEGMATCH_IMGSIM_COST_COLOR_SCALE);
        ...
        if(nOffsetColIdx>=0 && nOffsetColIdx<nCols && m_aROIs[nCamIdx^1](nRowIdx,nOffsetColIdx)) {
            ...
            const ValueType tAddedFGDistUnaryCost = ValueType((fCurrOffsetFGDist+fInitOffsetFGDist*fInitDistScale)*fInterSpectrScale*STEREOSEGMATCH_SHPDIST_COST_SCALE);
            ...
            const ValueType tAddedBGDistUnaryCost = ValueType((fCurrOffsetBGDist+fInitOffsetBGDist*fInitDistScale)*fInterSpectrScale*STEREOSEGMATCH_SHPDIST_COST_SCALE);
...
inline void StereoSegmMatcher::GraphModelData::calcStereoMoveCosts(size_t nCamIdx, InternalLabelType nNewLabel) const {
    ...
    const bool bCalcPairwCosts = lv::getVerbosity()>=4;
    // @@@@@ openmp here?
    for(size_t nGraphNodeIdx=0; nGraphNodeIdx<m_anValidGraphNodes[nCamIdx]; ++nGraphNodeIdx) {
        const size_t nLUTNodeIdx = m_avValidLUTNodeIdxs[nCamIdx][nGraphNodeIdx];
        const NodeInfo& oNode = m_vNodeInfos[nLUTNodeIdx];
...
inline opengm::InferenceTermination StereoSegmMatcher::GraphModelData::infer(size_t nPrimaryCamIdx) {
    ...
    const int nMaxEdgesPerNode = 2;
    kolmogorov::qpbo::QPBO<ValueType> oStereoMinimizer((int)m_anValidGraphNodes[nPrimaryCamIdx],(int)m_anValidGraphNodes[nPrimaryCamIdx]*nMaxEdgesPerNode);
    kolmogorov::qpbo::QPBO<ValueType> oResegmMinimizer_Left((int)m_anValidGraphNodes[0],(int)m_anValidGraphNodes[0]*nMaxEdgesPerNode);
    kolmogorov::qpbo::QPBO<ValueType> oResegmMinimizer_Right((int)m_anValidGraphNodes[1],(int)m_anValidGraphNodes[1]*nMaxEdgesPerNode);
    ...
    while(++nMoveIter<=m_nMaxMoveIterCount && nConsecUnchangedLabels<m_nStereoLabels) {
        ...
        if((nMoveIter%STEREOSEGMATCH_DEFAULT_ITER_PER_RESEGM)==0) {
            ...
            for(size_t nResegmLoopIdx=0; nResegmLoopIdx<STEREOSEGMATCH_DEFAULT_RESEGM_PER_LOOP; ++nResegmLoopIdx) {
                for(size_t nCamIdx=0; nCamIdx<getCameraCount(); ++nCamIdx) {
                    ...
                    for(InternalLabelType nResegmAlphaLabel : {s_nForegroundLabelIdx,s_nBackgroundLabelIdx}) {
                        ...
                        if((tCurrResegmEnergy-atLastResegmEnergies[nCamIdx])==ValueType(0))
            ...
            const ValueType tCurrResegmEnergyTotal = std::accumulate(atLastResegmEnergies.begin(),atLastResegmEnergies.end(),ValueType(0));
            std::stringstream ssResegmEnergyDiff;
            if((tCurrResegmEnergyTotal-tLastResegmEnergyTotal)==ValueType(0))
                ssResegmEnergyDiff << "null";
            else
