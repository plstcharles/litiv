...
TEST(sc,regression_constr) {
    EXPECT_THROW_LV_QUIET(std::make_unique<ShapeContext>(0));
    EXPECT_THROW_LV_QUIET(std::make_unique<ShapeContext>(2,0));
    EXPECT_THROW_LV_QUIET(std::make_unique<ShapeContext>(2,3,0.0f));
    EXPECT_THROW_LV_QUIET(std::make_unique<ShapeContext>(2,3,0.5f,0.1f));
}

TEST(sc,regression_default_params) {
    std::unique_ptr<ShapeContext> pShapeContext = std::make_unique<ShapeContext>();
...
TEST(sc,regression_full_compute) {
    std::unique_ptr<ShapeContext> pShapeContext = std::make_unique<ShapeContext>();
    ...
    if(lv::checkIfExists(TEST_CURR_INPUT_DATA_ROOT "/test_sc.bin")) {
        cv::Mat_<float> oRefDescs = lv::read(TEST_CURR_INPUT_DATA_ROOT "/test_sc.bin");
        ...
        for(int i=0; i<(int)vTargetPts.size(); ++i) {
            ...
            for(int j=0; j<nDescSize; ++j)
                ASSERT_FLOAT_EQ(aDesc1[j],aDesc2[j]);
        }
    }
    else
        lv::write(TEST_CURR_INPUT_DATA_ROOT "/test_sc.bin",oOutputDescs);
