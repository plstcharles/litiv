...
namespace vptz {
    ...
    public:
        ...
        //! reduce the time of reading frames in hard disk  . returns the current frame from the video sequence given the current FoV; specific frames can be obtained by using Set(...) prior to calling
    ...
    private:
		...
		std::chrono::system_clock::time_point firstTickTYC;
		std::chrono::system_clock::time_point executionEndTimeTYC;
		std::chrono::system_clock::time_point executionBeginTimeTYC;
		double d_executionEndTimeTYC;
		double d_executionBeginTimeTYC;
    ...
    public:
        ...
        //!Get the execution time (and communication delay&sleep time) of every loop, and calculate the next frame Id. moves the camera to the expected target position (if needed), and returns the next image from the tracking sequence (or an empty frame when it is over)
