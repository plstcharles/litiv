...
function [ output ] = read_lvmat( file_path, file_type )
    ...
    if strcmp(file_type,'plaintext')
        ...
        output = permute(reshape(output,fliplr(anSizes')),fliplr(1:nDims));
        fclose(fd);
    elseif strcmp(file_type,'binary')
        ...
        error('unrecognized mat archive type flag');
    end
end