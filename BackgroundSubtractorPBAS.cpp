...
#define R_OFFST (0.0000f)
#define R_SCALE (5.0000f)
...
BackgroundSubtractorPBAS::BackgroundSubtractorPBAS(	 int nInitColorDistThreshold
	...
	curr_debug_id = ++s_debug_id;
}

BackgroundSubtractorPBAS::~BackgroundSubtractorPBAS() {}

void BackgroundSubtractorPBAS::initialize(const cv::Mat& oInitImg) {
...
void BackgroundSubtractorPBAS::operator()(cv::InputArray _image, cv::OutputArray _fgmask, double learningRateOverride) {
	...
	for(int y=0; y<m_oImgSize.height; ++y) {
		for(int x=0; x<m_oImgSize.width; ++x) {
			...
			int nMinDist=nChannelSize;
			float* pfCurrDistThresholdFactor = (float*)(m_oDistThresholdFrame.data+flt32_idx);
			const int nCurrDistThreshold = (int)((*pfCurrDistThresholdFactor)*m_nDefaultColorDistThreshold);
...
#endif //!BGSPBAS_USE_GRADIENT_COMPLEMENT
				if(nTotDist<=nCurrDistThreshold) {
					if(nMinDist>nTotDist)
						nMinDist = nTotDist;
					nGoodSamplesCount++;
				}
#if BGSPBAS_USE_GRADIENT_COMPLEMENT
