			...
			for(size_t n=0; n<(s_nSamplesInitPatternWidth*s_nSamplesInitPatternHeight); ++n) {
				...
				for(nLocalWordIdx=0;nLocalWordIdx<m_nLocalWords;++nLocalWordIdx) {
					...
					if(pCurrLocalWord
							...
							&& ((pCurrLocalWord->nDescBITS<s_nDescDistTypeCutoff_1ch || nSampleIntraDescBITS<s_nDescDistTypeCutoff_1ch)?hdist_ushort_8bitLUT(nSampleIntraDesc,pCurrLocalWord->nDesc):gdist_ushort_8bitLUT(nSampleIntraDesc,pCurrLocalWord->nDesc))<=nDescDistThreshold) {
			...
			if(fPotentialLocalWordsWeightSum>=LWORD_WEIGHT_SUM_THRESHOLD) {
				...
				*pfCurrMeanSegmRes = ((*pfCurrMeanSegmRes)*(BGSCBLBSP_N_SAMPLES_FOR_MEAN-1))/BGSCBLBSP_N_SAMPLES_FOR_MEAN;
				...
				if(m_aapLocalDicts[idx_rand_ldict]) {
					...
					if((n_rand%nCurrLocalWordNeighborSpreadRate)==0 ||
						...
						(fRandMeanSegmRes>BGSCBLBSP_GHOST_DETECTION_S_MIN && fRandMeanLastDist<BGSCBLBSP_GHOST_DETECTION_D_MAX && (n_rand%4)==0)) {
						...
						if(fPotentialRandLocalWordsWeightSum<LWORD_WEIGHT_SUM_THRESHOLD) {
							...
							pRandLocalWord->nOccurrences = (size_t)(LWORD_WEIGHT_OFFSET*fMaxLocalWordWeight*0.25f);
			...
			while(nLocalWordIdx<m_nLocalWords && fPotentialLocalWordsWeightSum<LWORD_WEIGHT_SUM_THRESHOLD) {
				...
				for(size_t c=0;c<3; ++c) {
					...
					if(nColorDist>nCurrSCColorDistThreshold)
						goto failedcheck3ch;
					size_t nIntraDescDist, nInterDescDist;
					if(pCurrLocalWord->anDescBITS[c]<s_nDescDistTypeCutoff_1ch || anCurrIntraDescBITS[c]<s_nDescDistTypeCutoff_1ch) {
