...
void BackgroundSubtractorCBLBSP::refreshModel(size_t nBaseOccCount, size_t nOverallMatchOccIncr, size_t nUniversalOccDecr) {
	...
	if(m_nImgChannels==1) {
		for(size_t k=0; k<nKeyPoints; ++k) {
			...
			if(m_oFGMask_last_dilated.data[idx_orig_uchar]>0)
				continue;
			...
			for(size_t n=0; n<nLocalIters; ++n) {
				...
				if(m_oFGMask_last_dilated.data[idx_sample_uchar]>0)
					continue;
		...
		cv::imshow("m_oFGMask_last_dilated",m_oFGMask_last_dilated);
		cv::waitKey(0);
		for(size_t k=0; k<nKeyPoints; ++k) {
			const int y_orig = (int)m_voKeyPoints[k].pt.y;
			const int x_orig = (int)m_voKeyPoints[k].pt.x;
		...
		for(size_t k=0; k<nKeyPoints; k+=nLocalDictIterIncr) { // <=(m_nGlobalWords) gwords from (m_nGlobalWords) equally spaced keypoints
			...
			for(nGlobalWordIdx=0;nGlobalWordIdx<m_nGlobalWords;++nGlobalWordIdx) {
				...
				if(pCurrGlobalWord
						&& L1dist_uchar(pCurrGlobalWord->anColor,pRefBestLocalWord->anColor)<=nCurrTotColorDistThreshold
				...
				while(nLookupMapIdx<nKeyPoints && (size_t)(m_apGlobalWordListIter_3ch-m_apGlobalWordList_3ch)<m_nGlobalWords) {
					if(m_aapLocalDicts[nLookupMapIdx*m_nLocalWords] && oGlobalDictPresenceLookupMap.data[nLookupMapIdx]<UCHAR_MAX && !m_oFGMask_last_dilated.data[nLookupMapIdx]) {
						...
						for(nGlobalWordIdx=0;nGlobalWordIdx<m_nGlobalWords;++nGlobalWordIdx) {
							pCurrGlobalWord = (GlobalWord_3ch*)m_apGlobalDict[nGlobalWordIdx];
							if(pCurrGlobalWord
									&& L1dist_uchar(pCurrGlobalWord->anColor,pRefLocalWord->anColor)<=nTotColorDistThreshold
...
void BackgroundSubtractorCBLBSP::operator()(cv::InputArray _image, cv::OutputArray _fgmask, double learningRateOverride) {
	...
	if(m_nImgChannels==1) {
		for(size_t k=0; k<nKeyPoints; ++k) {
			...
			m_oGhostRegionMask.data[idx_uchar] = (((*pfCurrMeanRawSegmRes)>BGSCBLBSP_GHOST_DETECTION_SAVG_MIN || (*pfCurrMeanFinalSegmRes)>BGSCBLBSP_GHOST_DETECTION_ZAVG_MIN) && fCurrDistFactor<BGSCBLBSP_GHOST_DETECTION_DMIX_MAX && (*pfCurrMeanLastDist)<BGSCBLBSP_GHOST_DETECTION_DLST_MAX)?255:0;
			m_oHighVarRegionMask.data[idx_uchar] = (((*pfCurrMeanRawSegmRes)>BGSCBLBSP_HIGH_VAR_DETECTION_SAVG_MIN || (*pfCurrMeanFinalSegmRes)>BGSCBLBSP_HIGH_VAR_DETECTION_ZAVG_MIN) && fCurrDistFactor>BGSCBLBSP_HIGH_VAR_DETECTION_DMIX_MIN && (*pfCurrMeanLastDist)>BGSCBLBSP_HIGH_VAR_DETECTION_DLST_MIN)?255:0;
			const size_t nCurrWordOccIncr = (m_oGhostRegionMask.data[idx_uchar]>0)?std::max((size_t)16,nWordOccIncr):nWordOccIncr;
			...
			*pfCurrMeanLastDist = ((*pfCurrMeanLastDist)*(nSamplesForMean-1) + ((float)(absdiff_uchar(nLastColor,nCurrColor))/s_nColorMaxDataRange_1ch+(float)(hdist_ushort_8bitLUT(nLastIntraDesc,nCurrIntraDesc))/s_nDescMaxDataRange_1ch)/2)/nSamplesForMean; // @@@@ add bit trick?
			...
			if(fPotentialLocalWordsWeightSum>=fLocalWordsWeightSumThreshold) {
				...
				*pfCurrMeanRawSegmRes = ((*pfCurrMeanRawSegmRes)*(nSamplesForMean-1))/nSamplesForMean;
				...
				if(m_aapLocalDicts[idx_rand_ldict]) {
					...
					//const float fRandMeanRawSegmRes = *((float*)(m_oMeanRawSegmResFrame.data+idx_rand_flt32));
					...
					if((n_rand%nCurrLocalWordNeighborSpreadRate)==0 || ((n_rand%4)==0 && (m_oGhostRegionMask.data[idx_rand_uchar]>0))) {
			...
			if((fCurrDistFactor>BGSCBLBSP_R2_OFFST && m_oBlinksFrame.data[idx_uchar]>0) || (m_oHighVarRegionMask.data[idx_uchar]>0)) {
