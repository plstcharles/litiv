...
int main(int /*argc*/, char **/*argv*/) {
    try {
        ...
        for(int nCurrFrameIndex=0; nCurrFrameIndex<nFrameCount; ++nCurrFrameIndex) {
            ...
            lv::putText(oTransformedSourceOverlay,"Estimated Transformation",cv::Point(20,20),cv::FONT_HERSHEY_PLAIN,0.8,cv::Scalar_<uchar>::all(255),1);
            cv::Mat oGTTransformedSourceOverlay = (USE_THERMAL_TO_VISIBLE_PROJ?oSource_VISIBLE:oSource_THERMAL)+oGTTransformedSource;
            lv::putText(oGTTransformedSourceOverlay,"GT Transformation",cv::Point(20,20),cv::FONT_HERSHEY_PLAIN,0.8,cv::Scalar_<uchar>::all(255),1);
            cv::Mat oTransformedContoursOverlay = (USE_THERMAL_TO_VISIBLE_PROJ?oContours_VISIBLE:oContours_THERMAL)+oTransformedContours;
            lv::putText(oTransformedContoursOverlay,"Estimated Transformation",cv::Point(20,20),cv::FONT_HERSHEY_PLAIN,0.8,cv::Scalar_<uchar>::all(255),1);
            ...
            lv::putText(oGTTransformedContoursOverlay,"GT Transformation",cv::Point(20,20),cv::FONT_HERSHEY_PLAIN,0.8,cv::Scalar_<uchar>::all(255),1);
            cv::Mat oTransformedPolyMatOverlay; cv::cvtColor(oPolyMat/2+oTransformedPolyMat/2,oTransformedPolyMatOverlay,CV_GRAY2BGR);
            lv::putText(oTransformedPolyMatOverlay,"Estimated Transformation",cv::Point(20,20),cv::FONT_HERSHEY_PLAIN,0.8,cv::Scalar_<uchar>::all(255),1);
            ...
            lv::putText(oGTTransformedPolyMatOverlay,"GT Transformation",cv::Point(20,20),cv::FONT_HERSHEY_PLAIN,0.8,cv::Scalar_<uchar>::all(255),1);
