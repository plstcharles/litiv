...
namespace litiv {
    ...
    static inline STDMETHODIMP initMatFromMediaType(const AM_MEDIA_TYPE* pmt, cv::Mat& oOutput, bool* pbVFlip=nullptr) {
        if(!pmt)
            return E_POINTER;
    ...
    struct DShowFrameGrabber : ISampleGrabberCB {
        ...
        inline int64_t GetLatestFrameIdx() const {
            return m_nTotFramesProcessed;
        }

        inline int64_t GetLatestFrame(cv::Mat& oOutput, bool bVFlip=false) const {
        ...
        inline void SetFrameCallback(std::function<void(const cv::Mat&)> lCallback, bool bKeepInternalCopy=false) {
            std::lock_guard<std::mutex> oLock(m_oMutex);
    ...
    private:
        ...
        virtual STDMETHODIMP BufferCB(double /*dSampleTime*/, BYTE* /*pBuffer*/, long /*nBufferLen*/) override {
    ...
    struct DShowCameraGrabber {
        ...
        inline int64_t GetLatestFrameIdx() const {
            return m_bIsConnected?m_pFrameGrabber->GetLatestFrameIdx():-1;
        }

        inline int64_t GetLatestFrame(cv::Mat& oOutput, bool bVFlip=false) const {
            return m_bIsConnected?m_pFrameGrabber->GetLatestFrame(oOutput,bVFlip):-1;
        }

        inline bool SetFrameCallback(std::function<void(const cv::Mat&)> lCallback, bool bKeepInternalCopy=false) {
            if(m_bIsConnected)
