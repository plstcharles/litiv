...
ofdis::PatGridClass::PatGridClass(
    ...
    for(int x=0; x<nopw; ++x) {
        for(int y=0; y<noph; ++y) {
            ...
            p_init[i].setZero();
            pat.push_back(new PatClass(cpt,cpo,op,patchid));
            patchid++;
        }
    }
}
...
void ofdis::PatGridClass::SetComplGrid(PatGridClass *cg_in) {
    cg = cg_in;
}

void ofdis::PatGridClass::InitializeGrid(const float* im_ao_in, const float* im_ao_dx_in, const float* im_ao_dy_in) {
    im_ao = im_ao_in;
...
void ofdis::PatGridClass::SetTargetImage(const float* im_bo_in, const float* im_bo_dx_in, const float* im_bo_dy_in) {
    im_bo = im_bo_in;
...
void ofdis::PatGridClass::Optimize() {
#if USING_OPENMP
...
void ofdis::PatGridClass::InitializeFromCoarserOF(const float* flow_prev) {
#if USING_OPENMP
    #pragma omp parallel for schedule(dynamic,10)
#endif //USING_OPENMP
...
void ofdis::PatGridClass::AggregateFlowDense(float *flowout) const {
